...
/* maximum number of resource instances for a classM supported */
#define MAX_INSTS_PER_CLASS	8


struct res_template {
    int classM;				/* matching resource classM: insts
					   with this resource classM will be
...
struct res_pool {
  ...
  /* res classM -> res template mapping table, lists are NULL terminated */
...
struct res_template *res_get(struct res_pool *pool, int classM);
